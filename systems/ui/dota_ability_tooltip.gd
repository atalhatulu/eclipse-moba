...
func _build_ui() -> void:
	...
	style.border_color = Color(0.25, 0.38, 0.52, 1.0) # Steel Slate Blue
	...
	slot_badge_label.add_theme_font_size_override("font_size", 12)
	slot_badge_label.add_theme_color_override("font_color", Color(0.35, 0.80, 1.0))
	header_h.add_child(slot_badge_label)
	
	level_label = Label.new()
	level_label.text = "SVY 1/4"
	level_label.add_theme_font_size_override("font_size", 11)
	level_label.add_theme_color_override("font_color", Color(0.65, 0.72, 0.8))
	header_h.add_child(level_label)
	
	var sep1 = ColorRect.new()
	sep1.custom_minimum_size = Vector2(0, 1)
	sep1.color = Color(0.2, 0.28, 0.38, 0.6)
	...
	prop_grid.add_theme_constant_override("v_separation", 2)
	main_v.add_child(prop_grid)
	
	target_type_val = _add_prop_row(prop_grid, "🎯 Hedef:", "Birim Hedefli")
	target_filter_val = _add_prop_row(prop_grid, "🛡️ Etki:", "Düşmanlar & Orman")
	damage_type_val = _add_prop_row(prop_grid, "⚡ Hasar:", "Fiziksel Hasar")
	bkb_pierce_val = _add_prop_row(prop_grid, "👑 Büyü Bağışıklığı:", "Delmez")
	
	var sep2 = ColorRect.new()
	sep2.custom_minimum_size = Vector2(0, 1)
	sep2.color = Color(0.2, 0.28, 0.38, 0.6)
	...
	# 5. Advanced Mechanics Box (Always Visible, Concise & Informative)
	mechanics_box = PanelContainer.new()
	var m_style = StyleBoxFlat.new()
	m_style.bg_color = Color(0.08, 0.12, 0.16, 0.95)
	m_style.border_width_left = 3
	m_style.border_color = Color(0.3, 0.85, 0.95)
	m_style.content_margin_left = 8
	m_style.content_margin_right = 8
	m_style.content_margin_top = 5
	m_style.content_margin_bottom = 5
	m_style.corner_radius_top_right = 4
	m_style.corner_radius_bottom_right = 4
	mechanics_box.add_theme_stylebox_override("panel", m_style)
	main_v.add_child(mechanics_box)
	
	var m_vbox = VBoxContainer.new()
	m_vbox.add_theme_constant_override("separation", 2)
	mechanics_box.add_child(m_vbox)
	
	var m_hdr = Label.new()
	m_hdr.text = "🔍 MEKANİK & ETKİLEŞİM NOTLARI"
	m_hdr.add_theme_font_size_override("font_size", 9)
	m_hdr.add_theme_color_override("font_color", Color(0.35, 0.88, 1.0))
	...
	mechanics_label.add_theme_font_size_override("font_size", 9)
	...
	sep3.color = Color(0.2, 0.28, 0.38, 0.6)
	main_v.add_child(sep3)
	
	# 6. Footer (Cooldown & Mana)
	var footer_box = HBoxContainer.new()
	footer_box.add_theme_constant_override("separation", 16)
	main_v.add_child(footer_box)
	
	cooldown_val_label = Label.new()
	cooldown_val_label.text = "⏳ 0.0 sn"
	cooldown_val_label.add_theme_font_size_override("font_size", 10)
	cooldown_val_label.add_theme_color_override("font_color", Color(0.8, 0.85, 0.9))
	footer_box.add_child(cooldown_val_label)
	
	mana_val_label = Label.new()
	mana_val_label.text = "💧 0"
	mana_val_label.add_theme_font_size_override("font_size", 10)
	mana_val_label.add_theme_color_override("font_color", Color(0.4, 0.8, 1.0))
	footer_box.add_child(mana_val_label)
...
func show_ability(ab: AbilityResource, cur_level: int, slot_key: String) -> void:
	...
	if ab.cast_range > 0.0:
		_add_stat_line(stats_vbox, "🏹 MENZİL:", "%.0fm" % (ab.cast_range / 100.0), Color(0.75, 0.85, 0.95))
		
	if ab.applies_status_effect and ab.effect_duration > 0.0:
		var eff_name = "YAVAŞLATMA" if ab.effect_type == StatusEffect.EffectType.SLOW else "SABİTLEME (ROOT)"
		_add_stat_line(stats_vbox, "⌛ " + eff_name + ":", "%.1f sn" % ab.effect_duration, Color(0.4, 0.9, 1.0))
		
	# Footer Cooldown & Mana
	if ab.cooldowns.size() > 0:
		cooldown_val_label.text = "⏳ " + _format_scaling_array(ab.cooldowns, cur_level) + " sn"
	else:
		cooldown_val_label.text = "⏳ 0.0 sn"
