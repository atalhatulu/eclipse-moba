...
func _build_ui() -> void:
	...
	subtitle_lbl.text = "54 Özgün Kahraman — Nitelikler, Yetenekler ve Canlı Test"
	...
	_add_filter_btn(filters_hbox, FilterCategory.ALL, "TÜMÜ (54)", Color(0.8, 0.85, 0.9))
	_add_filter_btn(filters_hbox, FilterCategory.STRENGTH, "GÜÇ / STR (20)", Color(0.95, 0.35, 0.35))
	_add_filter_btn(filters_hbox, FilterCategory.AGILITY, "ÇEVİKLİK / AGI (18)", Color(0.35, 0.9, 0.45))
	_add_filter_btn(filters_hbox, FilterCategory.INTELLIGENCE, "ZEKA / INT (16)", Color(0.35, 0.75, 1.0))
	...
	btn_play_hero.text = "BU KAHRAMANLA OYNA / DÖNÜŞ"
	btn_play_hero.custom_minimum_size = Vector2(0, 42)
	var play_style = StyleBoxFlat.new()
	play_style.bg_color = Color(1.0, 0.73, 0.13, 0.95)
	...
	btn_set_bot.text = "BOT OLARAK SEÇ"
...
func _create_hero_card(def: HeroResource) -> Button:
	...
	tag_atk.text = "Menzilli" if def.attack_type == HeroResource.AttackType.RANGED else "Yakın"
...
func _add_mechanics_guide(def: HeroResource) -> void:
	var guide := HeroMechanicsGuideClass.get_guide(def)
	if guide.is_empty():
		return
	var panel = PanelContainer.new()
	var style = StyleBoxFlat.new()
	style.bg_color = Color(0.07, 0.12, 0.17, 0.96)
	style.border_width_left = 3
	style.border_color = Color(0.0, 0.88, 1.0, 0.85)
	...
	abilities_vbox.add_child(panel)
	var box = VBoxContainer.new()
	box.add_theme_constant_override("separation", 3)
	panel.add_child(box)
	var title = Label.new()
	title.text = "OYUN PLANI — PASİF, COMBO, KARŞI OYUN"
	title.add_theme_font_size_override("font_size", 10)
	title.add_theme_color_override("font_color", Color(0.0, 0.9, 1.0))
	box.add_child(title)
	_add_mechanics_line(box, "Pasif • %s: %s" % [guide["passive_name"], guide["passive_desc"]], Color(1.0, 0.82, 0.32))
	_add_mechanics_line(box, "Combo: " + guide["combo"], Color(0.62, 0.93, 1.0))
	_add_mechanics_line(box, "Karşı oyun: " + guide["counterplay"], Color(1.0, 0.63, 0.55))
	_add_mechanics_line(box, "Hedefleme: " + " | ".join(guide["targeting"]), Color(0.72, 0.80, 0.9))

func _add_mechanics_line(parent: Control, text: String, color: Color) -> void:
	var line = Label.new()
	line.text = text
...
func _add_ability_entry(ab: AbilityResource, slot_tag: String, header_col: Color) -> void:
	...
	if not ab.is_passive and not ab.cooldowns.is_empty():
		...
		cd_lbl.text = "BS: %.1fs | Mana: %.0f" % [ab.cooldowns[0], ab.mana_costs[0] if not ab.mana_costs.is_empty() else 0.0]
